...
class change_point_detector(nn.Module):
    ...
    def train(self, dataset, results_dir = None):
        ...
        flag_perturb_cp, flag_no_change_cp, perturb_train_count, flag_no_change_cp_arr, check_convergence, set_of_stable_cp, time_to_stop_perturb, static_cps = False, 0, 0, [], True,{}, False, [None]
        ...
        if self.opt.partition_method=='static_known_cp':
            static_cp_times = dataset[-1]
            static_cps = [ torch.where(data_time > t)[0][0].item() for t in static_cp_times]
        ...
        for epoch in range(num_epochs):
            ...
            time_loss, mark_loss, (CPD_objective, CPD_objective_binary), (solution, V,  predictions, (time_nll, type_nll), ratio_sums, log_ratio_data) = \
            self.forward(data_time, data_type, data_feat, num_changepoints + 1, device, partition_method=self.opt.partition_method, \
                flag_perturb_cp=flag_perturb_cp, perturb_train_count=perturb_train_count, static_cps = static_cps)
            # exit()
            ...
            if not self.opt.whether_global:
                if epoch % self.opt.save_interval == 0  or tot_flag == 50:  
                    ...
                    Utils.save(results_dir+'_CPD_objective',CPD_array)
                # Utils.save_CPD_model(self, self.opt, self.opt.save_intermediate_dir, intermediate=True, epoch = epoch+1)
...
class Diff_Seg_Iterator(change_point_detector):
    ...
    def train(self, dataset, results_dir = None):
        ...
        n, tot_flag, min_val_tot = self.len, 0, float(inf)
        self.changes = []
        epoch_array,lambdas_array, mark_array, loss_array, time_loss_array, \
        type_loss_array, mask_array, segment_predictor_array = [],[],[],[],[],[],[],[]
        data_time, data_type, data_feat, change_points\
         = self.dataset[0].to(device), self.dataset[1].to(device), self.dataset[5].to(device), self.dataset[4]
        num_params = sum(p.numel() for p in self.parameters() if p.requires_grad)
        ...
        for epoch in range(num_epochs):
            ...
            print('Epoch', epoch)
            # print('*************************************\n\n\n')
            loss, time_nll, type_nll, lambdas, mark, lambdas_latent, mark_latent, mask, segment_predictor = \
            self.forward(data_time, data_type, data_feat, num_changepoints + 1, device)
            # exit()
            # - ------------- CP detection --------------
            data_temp = data_time[1:]
            if not self.opt.pre_train_CPD_model:
                segment_map = torch.round(segment_predictor)
                change_points_detected = list(data_temp[(segment_map[1:]-segment_map[:-1])>0].cpu().detach().numpy())
                # print(change_points_detected)
                # exit()
                # change_points_detected = 0 # [ data_temp[(solution[i+1] > 0.5).cpu().detach().numpy()][0].item() for i in range(num_changepoints)]
                self.changes.append(change_points_detected) 
            ...
            lambdas_array.append(lambdas.cpu().detach().numpy())
            mark_array.append(mark.cpu().detach().numpy())
            loss_array.append(loss.item())  
            time_loss_array.append(torch.sum(time_nll).item())
            type_loss_array.append(torch.sum(type_nll).item())
            mask_array.append(mask.cpu().detach().numpy())
            segment_predictor_array.append(segment_predictor.cpu().detach().numpy())
            ...
            if epoch % self.opt.save_interval == 0  or tot_flag == 50:  
                if not self.opt.pre_train_CPD_model:
                    Utils.save(results_dir+"change_points",self.changes)
                    Utils.save(results_dir+"segment_map",{'mask':mask_array,'segment_predictor':segment_predictor_array})
                #---------------- Common plots -----------------------------------------------
                Utils.save(results_dir+"loss",{"time":time_loss_array,"type":type_loss_array})
                Utils.save(results_dir+'predictions',{'lambda':lambdas_array, 'marks':mark_array})
                #---------------Logging--------------------------------------------------------
                print("Epoch: {} Loss: {:.2f}".format(epoch, loss.item()))
                print(datetime.datetime.now().strftime("%a, %d %B %Y %H:%M:%S"))
                with open(self.opt.log_file, 'a') as f:
...
class Diff_Seg(Diff_Seg_Iterator):
    ...
    def __init__(self, opt, num_types=1, len_seq=1, len_feat=0):
        ...
        self.n_gamma=opt.diff_seg_n_gamma  
        self.w_gamma = opt.diff_seg_w_gamma
        ...
        if self.opt.pre_train_CPD_model == True:
            ...
            self.mu = torch.rand(opt.num_changepoints+1) # rand ? ********** dummy ********** # 
    ...
    def gamma(self,t,m):
        ...
        # print(K)
        # exit()
        U = t_new.repeat(K-1,1)
        M = m.reshape(-1,1).repeat(1,n)
        A = torch.max(torch.zeros_like(M),torch.min(torch.ones_like(M)*(1-self.w_gamma),M-self.w_gamma/2))
        B = torch.min(torch.ones_like(M), A+self.w_gamma)

        # print(U.shape,M.shape,A.shape,B.shape)
        # print(M[:,0],M[:,1])
        # print(B[:,0],B[:,1])
        # exit()
        flag_1 = (U<=A)
        flag_2 = torch.logical_and((U>A),(U<=M))
        flag_3 = torch.logical_and((U>M),(U<=B))
        flag_4 = (U>B)

        # exit()

        f1 = torch.zeros_like(U)
        f2 = torch.mul(torch.div(M-A,B-A),torch.pow( torch.div(U-A,M-A), self.n_gamma))
        f3 = 1 - torch.mul(torch.div((B-M),(B-A)), torch.pow( torch.div((B-U),(B-M)),self.n_gamma))
        f4 = torch.ones_like(U)

        F_TSP = flag_1 * f1 + flag_2 * f2 + flag_3 * f3 + flag_4 * f4 

        # exit()
        return (1.0/(K-1)) * torch.sum(F_TSP, axis=0)

    def forward(self, data_time, data_type, data_feat, num_part, device, partition_method = 'cvxpy', flag_perturb_cp=False, perturb_train_count=0, static_cps = []):
        n=data_time.shape[0]
        event_time = torch.unsqueeze(data_time, dim=0).type(torch.FloatTensor)
        ...
        if self.opt.pre_train_CPD_model == True:    
            ...
            time_nll = Utils.log_likelihood_array(lambdas_pretrained, event_time)
            type_nll = Utils.log_likelihood_mark_array(mark_pretrained[:-1], event_type[0][1:] - 1)
            time_loss = Utils.compute_time_loss(prediction[1], event_time)
            scale_time_loss = 10
            time_loss = time_loss/scale_time_loss

            # print(lambdas_pretrained.shape)
            # print(mark_pretrained.shape)
            # print(time_nll.shape)
            # print(type_nll.shape)
            # exit()
            total_loss = torch.sum(time_nll) + torch.sum(type_nll) + time_loss
            return total_loss, time_nll, type_nll, lambdas_pretrained, mark_pretrained, lambdas_pretrained, mark_pretrained, torch.zeros_like(lambdas_pretrained),torch.zeros_like(lambdas_pretrained)
        else:                    
            lambdas_latent = torch.zeros(num_part*V.shape[0],n).to(device)
            mark_latent = torch.zeros(num_part*V.shape[0],n,self.num_types).to(device)
            # print(lambdas_latent.shape)
            # print(mark_latent.shape)
            # exit()            
            exp_mu = torch.exp(self.mu)
            m = torch.cumsum(exp_mu, dim=0)[:-1]/torch.sum(exp_mu)
            # print(m.shape)
            # exit()
            segment_predictor = 1 + self.num_change_points * self.gamma(event_time,m) # size should be number of events

            # print(segment_predictor.shape)
            # exit()
            # number of partitions * number of events
            mask = torch.max(torch.zeros_like(lambdas_latent),1-torch.abs(segment_predictor.repeat(num_part,1) - torch.range(1,num_part).reshape(-1,1).repeat(1,n)))
            # print(mask.shape)
            # exit()
            for i in range(num_part):
                lambdas_latent[i] = nn.Softplus()(self.linear[i](V)).squeeze()
                mark_latent[i] = self.linear_mark[i](torch.cat((V, event_feat),dim=-1))
                # torch.squeeze(F.log_softmax(self.linear_mark[i](torch.cat((V, event_feat),dim=-1)), dim=-1),0)

            lambdas = torch.sum(torch.mul(mask,lambdas_latent), axis=0)
            # print(mask.shape)
            mask_mark = torch.unsqueeze(mask,dim=-1).repeat(1,1,mark_latent.shape[-1])
            # print(mask_new.shape)
            # exit()
            mark = torch.squeeze(F.log_softmax(torch.sum(torch.mul(mask_mark,mark_latent), axis=0), dim=-1),0)
            
            # print(lambdas.shape, mark.shape)
            # exit()
            time_nll = Utils.log_likelihood_array(lambdas, event_time)
            type_nll = Utils.log_likelihood_mark_array(mark[:-1], event_type[0][1:] - 1)

            # print(time_nll.shape, type_nll.shape)
            # exit()

            total_loss = torch.sum(time_nll) + torch.sum(type_nll)

            return total_loss, time_nll, type_nll, lambdas, mark, lambdas_latent, mark_latent, mask,segment_predictor
...
class differentiable_change_point_detector(change_point_detector_outer):
    ...
    def forward(self, data_time, data_type, data_feat, num_part, device, partition_method = 'cvxpy', flag_perturb_cp=False, perturb_train_count=0, static_cps = []):
        ...
        if self.opt.pre_train_CPD_model == True:
            ...
            # print('right')
            # exit()
            # time_loss, type_loss, (CPD_objective, CPD_objective_biary), (solution, V,  (lambdas, mark), (time_nll_sum, type_nll_sum), ratio_sums, log_ratio_data)
            ...
            if partition_method=='static':
                ...
                for i, cp_index  in enumerate(list_partitions):
                    ...
                    # exit()
            if partition_method=='static_known_cp':
                solution = torch.zeros(num_part-1,n-1).to(device)
                for i, cp_index  in enumerate(static_cps):
                    solution[i,cp_index:]=1

...
def main_call(av, dataset, data, config, config_parameters, i):
    ...
    ap.add_argument("--src_method",                             type=str,   default='GLR_Hawkes')
    ap.add_argument("--src_folder",                             type=str,   default='')
    ...
    ap.add_argument("--diff_seg_n_gamma",                       type=int, default = int(config_parameters['BASELINES']['diff_seg_n_gamma']), help="diff_seg_n_gamma")
    ap.add_argument("--diff_seg_w_gamma",                       type=float, default = float(config_parameters['BASELINES']['diff_seg_w_gamma']), help="diff_seg_w_gamma")        

    av =  ap.parse_args()
    av.dataset = dataset
    av.data = data
    av.method_short = Utils.map_method_to_short(av.method, av.pre_train_CPD_model)
    av.src_method_short = Utils.map_method_to_short(av.src_method, av.pre_train_CPD_model)

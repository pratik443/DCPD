...
from models.change_point_methods import differentiable_change_point_detector, Score_statistics, GLR_Hawkes, stochastic_greedy_cpd, Greedy_selection, Diff_Seg
...
class learn_change_point_detection(object):
    ...
    def __init__(self, av, config):
        ...
        self.av.score_file = config['load_scores'].replace('dataset',av.data).replace('algorithm',av.src_method_short).replace('seed',str(av.seed*7)).replace('sequence_num',str(av.seq_no)).replace('src_folder',av.src_folder)
        # TO DO::Rename initial dataset prefix
        # 
        # 
    ...
    def train(self):
        ...
        if self.av.partition_method == 'static_known_cp':
            scores = Utils.load_data(self.av.score_file)
            # exit()
            cp_times = Utils.sorted_change_points(scores, self.av.num_changepoints)
            # print(type(dataset))
            # exit()
            dataset = dataset + (cp_times,)
            # print(type(dataset))
            # print(dataset[-1])
            # exit()
